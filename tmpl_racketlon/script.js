...
(function ($) {
    'use strict';
    $(onLoad);
    $(window).on('resize', applyImageScalling);
    function onLoad() {
    ...
    function scaling(c, img) {
        ...
        var imgTargetClass = c.attr('data-imagescaling-target') ? c.attr('data-imagescaling-target') : '';
        var imgClass = img.attr('class');
        if (imgSrc && imgClass) {
            $(c).css('width', '').css('height', '');
            imgClass.replace(imgTargetClass, '')
                .replace(new RegExp('data-control-id-\\d+', 'g'), '')
                .trim();
            var width = img.width();
            ...
            c.find('.bd-imagescaling-img').remove();
            if ($(c) && $(c).width() === 0) {
            ...
            $('<div class="bd-imagescaling-img ' + imgClass + '"></div>').css('background-image', 'url(' + imgSrc + ')').appendTo(img.parent());
        }
    }
})(jQuery);
